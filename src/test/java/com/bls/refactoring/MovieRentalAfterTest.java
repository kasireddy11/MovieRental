...
public class MovieRentalAfterTest extends TestCase {
	...
	/*
	 * Implpemented this test case after adding new change AnimatedClass
	 */
	public void testAnimatedMovie() {
		Rental rental = new Rental(new AnimatedMovie(movieTitleInTime), 1);
		customer.addRental(rental);
		assertEquals("Rental record for : Kasi.....\n\tIn time: 90.0\n"
				+ "Amount owed is: 90.0\nYou have earned 1 frequent renter points", customer.statement());
	}
	...
	/*
	 * Implpemented this test case after adding new change tableStatement
	 */
	public void testTableStatement() {
		Rental rental = new Rental(new RegularMovie(movieTitleIdentityThief), 5);
		customer.addRental(rental);
		rental = new Rental(new ChildrenMovie(movieTitleWallE), 4);
		customer.addRental(rental);
		rental = new Rental(new NewReleaseMovie(movieTitleInTime), 7);
		customer.addRental(rental);
		rental = new Rental(new AnimatedMovie(movieTitleInTime), 5);
		customer.addRental(rental);
		assertEquals("Name : Kasi\n" + "\tIdentity Thief: 325.0\n" + "\tWall E: 150.0\n" + "\tIn time: 1050.0\n"
				+ "\tIn time: 290.0\n" + "Amount owed is: 1815.0\n" + "Renter points : 5", customer.tableStatement());
	}
}